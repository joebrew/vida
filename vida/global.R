...
if('saved_data.RData' %in% dir()){
  ...
  for(j in 1:ncol(gambia_translated)){
    ...
    if(this_column %in% gambia_dictionary$Variable){
      if(!is.na(gambia_dictionary$ValueList[gambia_dictionary$Variable == this_column][1])){
        # Get dictionary
        the_dict <- 
          gambia_dictionary$ValueList[gambia_dictionary$Variable == this_column][1]
  ...
  for(j in 1:ncol(gambia_translated)){
    ...
    if(this_column %in% gambia_dictionary$Variable){
      names(gambia_translated)[j] <-
        gambia_dictionary$Question[gambia_dictionary$Variable == this_column][1]
    }
  }
  save(gambia_translated,
       gambia_dictionary,
       gambia_in_depth,
       file = 'saved_data.RData')
}
